...
export const SubscriptionModal: React.FC<SubscriptionModalProps> = ({
  ...
  const perks = translations[locale].subscription.perks;
      ...
      <div className="relative w-full max-w-lg rounded-2xl bg-[#151724] border border-[#282b3e] shadow-2xl shadow-black/80 p-6 overflow-hidden">
        ...
        <div className="flex items-center gap-2.5 mb-2">
          ...
          <div>
            <h2 className="text-base font-bold text-slate-100 flex items-center gap-2">
              HatePPT <span className="bg-gradient-to-r from-amber-400 to-yellow-200 bg-clip-text text-transparent">{t('subscription.title')}</span>
              {isPro && (
            ...
            <p className="text-xs text-[#8a91a8]">{t('subscription.subtitle')}</p>
          </div>
        </div>
        ...
        {currentUser && (
          <div className="mt-3 py-2 px-3 rounded-lg bg-[#181a28] border border-[#232536] flex items-center justify-between text-xs">
            <div className="flex items-center gap-2 text-slate-300">
              ...
              <span>{t('subscription.currentAccount')} <strong className="text-slate-100">{currentUser.nickname}</strong> ({currentUser.email})</span>
            </div>
            <span className={`px-2 py-0.5 rounded text-[10px] font-semibold ${
              isPro ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' : 'bg-[#202336] text-[#8a91a8]'
            }`}>
              {isPro ? t('subscription.proMember') : t('subscription.freeUser')}
            </span>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="p-3.5 rounded-xl border border-[#282c40] bg-[#191c2b] flex flex-col justify-between relative hover:border-[#353a54] transition-colors">
            <div>
              <div className="text-xs text-[#8a91a8] font-medium mb-1">{t('subscription.monthlyTitle')}</div>
              <div className="flex items-baseline gap-1">
                <span className="text-2xl font-bold text-slate-100">{t('subscription.monthlyPrice')}</span>
                <span className="text-xs text-[#717894]">{t('subscription.monthlyUnit')}</span>
              </div>
            </div>
            <div className="text-[11px] text-[#8a91a8] mt-2">{t('subscription.monthlyDesc')}</div>
          </div>
          ...
          <div className="p-3.5 rounded-xl border-2 border-amber-500/70 bg-amber-500/10 flex flex-col justify-between relative shadow-lg shadow-amber-950/20">
            <span className="absolute -top-2.5 right-3 px-2 py-0.5 rounded-full text-[10px] font-bold bg-gradient-to-r from-amber-500 to-yellow-400 text-slate-950">
              {t('subscription.annualBadge')}
            </span>
            <div>
              <div className="text-xs text-amber-400 font-medium mb-1">{t('subscription.annualTitle')}</div>
              <div className="flex items-baseline gap-1">
                <span className="text-2xl font-bold text-amber-300">{t('subscription.annualPrice')}</span>
                <span className="text-xs text-[#717894]">{t('subscription.annualUnit')}</span>
              </div>
            </div>
            <div className="text-[11px] text-amber-200/70 mt-2">{t('subscription.annualDesc')}</div>
          </div>
        </div>
          ...
          {currentUser ? (
            isPro ? (
              ...
              >
                ...
                <span>{t('subscription.btnUpgrade')}</span>
              </button>
            )
          ) : (
            <button
            ...
            >
              <Sparkles size={14} />
              <span>{t('subscription.btnNeedLogin')}</span>
              <ArrowRight size={14} />
            </button>
          )}
          ...
          {currentUser && onLogout && (
            <div className="pt-2 text-center">
              <button
                type="button"
                onClick={() => {
                  if (window.confirm(t('subscription.logoutConfirm', { email: currentUser.email }))) {
                    onLogout();
                    onClose();
                  }
                }}
              ...
              >
                <LogOut size={12} />
                <span>{t('subscription.btnLogout')}</span>
              </button>
            </div>
          )}
        </div>
